...
var KMA = (function ($, $n) {
  return $.extend($n, {
    ...
    initCallback: function (timeout) {
      try {
        ...
        if (window.kmacb_form_selector != undefined) {
          ...
          $(window.kmacb_form_selector).on(
            'click',
            function (event, disableTrigger) {
              if (disableTrigger == undefined || !disableTrigger) {
    ...
    checkAndSetCountryField: function (country) {
      if (
        !$('select[name=country]').length &&
        !$('input[name=country]').length
      ) {
        ...
        setTimeout(function start_kmacomebacker() {
          ...
          $(window).on('mouseout', function (event) {
            if (event.pageY - $(window).scrollTop() < 1 && comebacker) {
              if (
                window.customPopupShowed != undefined &&
                window.customPopupShowed === true
              ) {
                return
              }
              if ($('.modal:visible').length) {
    ...
    initComebackerAlert: function (KMAText) {
      ...
      window.onbeforeunload = function (evt) {
        if (current.showComebackerAlert) {
          current.showComebackerAlert = false
          $('form').append(
            '<input type="hidden" name="aftercomebacker" value="1" />'
          )
    ...
    initFireMetric: function () {
      ...
      $(document).on(
        'click',
        '[data-trigger=click]',
        function (e, disableTrigger, generateByCB) {
          if (!generateByCB) {
...
$(function () {
  ...
  $('form').on('submit', function (event) {
    var form = $(this)
    if (
      form.closest('#kmacb-form').length ||
      form.closest('.mfp-content').length ||
      form.closest('.puForm').length ||
      form.closest('.modal').length
    ) {
...
$(function () {
  ...
  setTimeout(function () {
    if (
      $('.country_select:visible').length &&
      $('.country_select:visible').val() != country
    ) {
      $('.country_select:visible').val(country)
      $('.country_select:visible').trigger('change')
    }
  }, 500)
})
